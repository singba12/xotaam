...
document.getElementById('logout-btn').addEventListener('click', async function() {
    try {
		...
		if (userConfirm) {
			...
			window.location.href = '/login'; // Remplacez par votre URL de page de connexion
		} 
    } catch (error) {
...
window.onload = function(){
	...
	if(connect != 1){window.location.href = '/login'}
	else{
		...
		let role = data[4];
		//alert(role);
		if(role === 'chauffeur'){window.location.href = '/driver'}
		return;
	}
}
